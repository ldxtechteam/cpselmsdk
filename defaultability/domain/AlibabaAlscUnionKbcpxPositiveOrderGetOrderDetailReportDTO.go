...
type AlibabaAlscUnionKbcpxPositiveOrderGetOrderDetailReportDTO struct {
    ...
    /*
        扩展信息。tpOrderId：卡券场景下用来找C端用户看到的卡券订单号，无需理解，只要匹配上就行；alscOrderId：卡券场景下用来找C端用户看到的卡券核销订单号，无需理解，只要匹配上就行；isSelfPurchase：是否字段，true/false ；cityName：城市，示例：上海 ；platformActivityType：活动类型，BRAND、CONSUMPTION，对应品牌日和消费日 ；attrOrderDesc：是否归因订单，是/否；lockId评价有礼字段；commentFee评价返现金额     */
...
func (s *AlibabaAlscUnionKbcpxPositiveOrderGetOrderDetailReportDTO) SetActivityInfoRemarkList(v string) *AlibabaAlscUnionKbcpxPositiveOrderGetOrderDetailReportDTO {
    s.ActivityInfoRemarkList = &v
    return s
}
func (s *AlibabaAlscUnionKbcpxPositiveOrderGetOrderDetailReportDTO) SetChannelRightId(v string) *AlibabaAlscUnionKbcpxPositiveOrderGetOrderDetailReportDTO {
    s.ChannelRightId = &v
    return s
}
func (s *AlibabaAlscUnionKbcpxPositiveOrderGetOrderDetailReportDTO) SetExtInfo(v string) *AlibabaAlscUnionKbcpxPositiveOrderGetOrderDetailReportDTO {
    s.ExtInfo = &v
    return s
}
